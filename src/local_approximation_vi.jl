mutable struct LocalApproximationRandomSolver{I<:LocalFunctionApproximator, RNG<:AbstractRNG} <: Solver
...
# Default constructor
function LocalApproximationRandomSolver(interp::I;
    ...
    return LocalApproximationRandomSolver(interp,max_iterations, belres, verbose, rng, is_mdp_generative, n_generative_samples)
end

# Unparameterized constructor just for getting requirements
function LocalApproximationRandomSolver()
    throw(ArgumentError("LocalApproximationRandomSolver needs a LocalFunctionApproximator object for construction!"))
end


# NOTE : We work directly with the value function
# And extract actions at the end by using the interpolation object
mutable struct LocalApproximationRandomPolicy{I<:LocalFunctionApproximator, RNG<:AbstractRNG} <: Policy
...
function LocalApproximationRandomPolicy(mdp::Union{MDP,POMDP},
                                                solver::LocalApproximationRandomSolver)
    return LocalApproximationRandomPolicy(deepcopy(solver.interp),ordered_actions(mdp),mdp,
                                                  solver.is_mdp_generative,solver.n_generative_samples,solver.rng)
end


@POMDP_require solve(solver::LocalApproximationRandomSolver, mdp::Union{MDP,POMDP}) begin
...
function POMDPs.solve(solver::LocalApproximationRandomSolver, mdp::Union{MDP,POMDP})
    ...
    policy = LocalApproximationRandomPolicy(mdp, solver)
    ...
    for i = 1 : max_iterations
        ...
        for (istate,s) in enumerate(interp_states)
            ...
            if isterminal(mdp, s)
                ...
                util /= length(sub_aspace)

                # Update this interpolant value
                interp_values[istate] = util
                util_diff = abs(util - old_util)
...
function POMDPs.value(policy::LocalApproximationRandomPolicy, s::S) where S
...
POMDPs.action(policy::LocalApproximationRandomPolicy, s::S) where S = rand(actions(policy.mdp, s))

# Compute the action-value for some state-action pair
# This is also used in the above function
function POMDPs.value(policy::LocalApproximationRandomPolicy, s::S, a::A) where {S,A}
